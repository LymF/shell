...
GENOME_REF="/home/lucasyago/remontagem_libs/GCF_000208745.1_Criollo_cocoa_genome_V2_genomic.fna"  # Caminho para o genoma de referência
...
for FILE in "$INPUT_DIR"/*_01.fasta; do
    ...
    # 1. Trimagem com Trim Galore!
    trim_galore --paired "$READ1" "$READ2" -o "$OUTPUT_DIR"
    
    TRIM_READ1="$OUTPUT_DIR/${SAMPLE}_01_val_1.fq"
    TRIM_READ2="$OUTPUT_DIR/${SAMPLE}_02_val_2.fq"
    
    # 2. Mapeamento contra o genoma de referência (Bowtie2)
    bowtie2 -x "$GENOME_REF" -1 "$TRIM_READ1" -2 "$TRIM_READ2" \
    ...
    # 7. Redução de redundância com CD-HIT e CAP3
